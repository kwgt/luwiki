...
use crate::mcp::tools::GetPageToolArgs;
...
///
/// # 戻り値
/// `get_page` の tool result を返す。
///
pub(crate) async fn execute(
    server: &LuwikiMcpServer,
    Parameters(args): Parameters<GetPageToolArgs>,
    ...
    /*
     * 既存 handler / service へ `get_page` を橋渡しする
     */
    let result = server.with_state_read(|state| {
        Ok(handler.handle_get_page(
            &auth,
            state.db(),
            address,
            &args.path,
        ...
        Ok(response) => {
            let content = Content::json(response).map_err(|error| {
                McpProtocolError::internal_error(
                    format!(
                        "failed to serialize get_page response: {error}"
                    ),
                    None,
                )
            })?;
