...
use crate::mcp::tools::ListPagesToolArgs;
...
///
/// # 戻り値
/// `list_pages` の tool result を返す。
///
pub(crate) async fn execute(
    server: &LuwikiMcpServer,
    Parameters(args): Parameters<ListPagesToolArgs>,
    ...
    /*
     * 既存 handler / service へ `list_pages` を橋渡しする
     */
    let result = server.with_state_read(|state| {
        Ok(handler.handle_list_pages(
            &auth,
            state.db(),
            address,
            &args.prefix,
        ...
        Ok(response) => {
            let content = Content::json(response).map_err(|error| {
                McpProtocolError::internal_error(
                    format!(
                        "failed to serialize list_pages response: {error}"
                    ),
                    None,
                )
            })?;
