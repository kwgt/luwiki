...
pub(crate) async fn execute(
    ...
    /*
     * 既存 handler / service へ `append_page` を橋渡しする
     */
    let result = server.with_state_read(|state| {
        Ok(handler.handle_append_page(
            &auth,
            state.db(),
            address,
            &args.path,
        ...
        Ok(response) => {
            let content = Content::json(response).map_err(|error| {
                McpProtocolError::internal_error(
                    format!(
                        "failed to serialize append_page response: {error}"
                    ),
                    None,
                )
            })?;
