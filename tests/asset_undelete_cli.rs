...
#[test]
///
/// asset undelete がアセットの削除を取り消せることを確認する。
///
/// # 注記
/// 1) テスト用ユーザを作成する
/// 2) APIでページとアセットを作成する
/// 3) asset delete を実行する
/// 4) asset undelete を実行する
/// 5) asset list の行が削除扱いではないことを確認する
fn asset_undelete_cli_restores_asset() {
    ...
    let page_id = create_page(&api_url, "/asset-undelete", "body");
    ...
    run_asset_delete(&db_path, &assets_dir, &asset_id);
    run_asset_undelete(&db_path, &assets_dir, &asset_id);

    let output = run_asset_list(&db_path, &assets_dir);
    let line = output
        .lines()
        .find(|value| value.contains("asset.bin"))
        .expect("asset line missing");
...
/// * `assets_dir` - アセットディレクトリのパス
/// * `target` - 対象アセットID
fn run_asset_delete(db_path: &Path, assets_dir: &Path, target: &str) {
    let exe = test_binary_path();
    let base_dir = db_path
        .parent()
        .expect("db_path parent missing");
...
///
/// asset undelete を実行する。
///
/// # 引数
/// * `db_path` - DBファイルのパス
/// * `assets_dir` - アセットディレクトリのパス
/// * `target` - 対象アセットID
fn run_asset_undelete(db_path: &Path, assets_dir: &Path, target: &str) {
    let exe = test_binary_path();
    let base_dir = db_path
        .parent()
        .expect("db_path parent missing");
    let output = Command::new(exe)
        ...
        .arg(target)
        .output()
        .expect("asset undelete failed");

    if !output.status.success() {
        panic!(
            "asset undelete failed: {}",
            String::from_utf8_lossy(&output.stderr)
        );
    }
}
