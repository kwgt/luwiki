...
- Basic認証およびBearer認証の詳細設計は別設計書で定義する
- 本書では、認証そのものの詳細手順やBearerトークン管理構造には踏み込まず、必要に応じて関連モジュールやデータモデルとの接点のみを扱う
...
本章は、`docs/REQUIREMENTS.md` に定義するページ作成・編集の手順ポリシーのうち、
ブラウザUIから人間が直接操作する場合の具体フローを定義するものである。

API および MCP のように、クライアントが保存対象の内容を要求時点で提示できる
機械操作については、本章の段階的手順をそのまま必須前提とはしない。
...
閲覧画面の新規作成ボタンからの遷移では `/edit/{page_path}?create=1` を使用し、
ドラフトページの作成は編集画面側で行うものとする。
...
- `instance_id` が存在しない既存データは `no-store` 扱いとする
  ...
  2. ロック解除トークンを保持している場合は以下の手順で処理する
      1. REST API `DELETE /api/pages/{page_id}/lock`を呼び出しロックの解除を行う
      2. セッションストレージからロック解除トークンを削除する
...
タブ固有ID生成処理は非同期関数化しUIを停滞をさせない実装にする。

---
## 19. 全文検索
インデックスライターインスタンスとインデックスリーダインスタンスをアプリケーションステートとしてシングルトンで保持し、必要に応じて各ハンドラで内容の更新／参照を行う。

### 19.1 インデックス追加
- 記事の書き込み時(REST APIの`PUT /api/pages/{page_id}/source[?amend={boolean}]`)が発行された場合に行う。
- 各リビジョンのインデックスを保持し全リビジョンを検索対象とする。但し、最新リビジョンを優先できる仕組みを設ける。
- 削除状態(ソフトデリート状態)の記事も検索対象とする。
- 記事の更新の場合は旧記事に相当するインデックスを削除してから、新記事のインデックスを追加する。
- インデックス更新処理は時間がかかる事を前提とし、タスクとして独立できるように実装の検討を行う。

### 19.2 インデックス削除
- 記事のソフトデリートではインデックスの削除を行わない。
- ハードデリート時にインデックスの削除を行う。
