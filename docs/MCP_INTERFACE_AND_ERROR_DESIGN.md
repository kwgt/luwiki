...
ページ由来resourceでは、current pathがBearerのpath prefix範囲外の場合、
エラーではなく一覧から除外する。固定組み込みresourceにはページ用
path prefix制約を適用しない。
...
- latest sourceのfront matterを再検証し、resource ID一致を確認する
...
ページ由来resourceのcurrent pathがBearerのpath prefix範囲外の場合は、
...
draft、soft delete、hard delete、path prefix範囲外は、
いずれも`-32602` / `not_found` / `resource not found`へ秘匿する。

resource URI形式として成立しない入力、境界空白、制御文字、
`/page/`配下のresource ID値制約違反は`invalid_input`とする。
ただしauthorityが異なるURI、未知の`/builtin/` ID、
`/builtin/`または`/page/`以外の既知authority配下pathは、
...
ページ由来resourceにはcurrent path基準でページ用path prefix制約を適用する。
