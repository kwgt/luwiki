...
5. import対象page ID群のprompt候補同期

prompt候補、MCP primitive名前索引、名前索引readinessはexportデータへ
直接収録しない。prompt定義の正本は、ページソースに含まれるfront matterと本文である。
...
`ExportBundle`はredbの派生データテーブルを表すモデルを含めない。
特に、次のMCP prompts内部データはexport対象外とする。

1. `PROMPT_CANDIDATE_TABLE`
2. `MCP_PRIMITIVE_NAME_TABLE`
3. `MCP_PRIMITIVE_NAME_STATE_TABLE`

これらはページソースから同期または再構成できるため、
prompt専用のJSON / JSONLエントリを追加しない。
...
promptのfront matterと本文は、通常のページソースとして
`revisions.jsonl`へ格納する。prompt候補、primitive名前索引、
名前索引readinessは収集対象に含めない。
...
12. 全リビジョンのfront matterを検証する
13. アセット実体を一時配置する
14. DBとMCP primitive名前索引を同一write transactionで反映する
15. DB commit後にアセット実体を本配置する
16. import対象page ID群のprompt候補を差分同期する

import完了後に常にprompt派生データを全件再構成する方式は採用しない。
対象page ID群の候補同期に失敗した場合は、その失敗を呼び出し元へ返し、
`derived rebuild --target prompts`または`all`を運用上の復旧手段として使用する。
...
### 9.9 front matterとMCP primitive名前検証

1. DB write transactionを開始する前に、import対象の全リビジョンについて
   front matterの構文とスキーマを検証する
2. DB反映時は、各importページのlatest sourceからMCP primitive名前キーを生成する
3. import対象ページ同士の同一primitive内名前重複を拒否する
4. import対象ページと既存ページの同一primitive内名前重複を拒否する
5. soft delete済み既存ページが予約しているprompt名も重複対象とする
6. prompt名はcase-sensitiveな完全一致で比較し、trim、小文字化、
   Unicode正規化を行わない
7. 名前重複時はページ正本とprimitive名前索引を含むimport transaction全体を
   rollbackし、部分反映しない
...
4. リビジョンをDBへ投入する
5. アセットメタデータをDBへ投入する
6. latest sourceからMCP primitive名前索引を同期する
7. DB write transactionをcommitする
8. アセット実体ファイルを本配置する
9. import対象page ID群のprompt候補を差分同期する
...
4. primitive名前索引をページ正本と同じtransactionで更新し、
   import対象内および既存ページとの名前重複を原子的に拒否するため
5. prompt候補は正本commit後に最新ソースから再計算する派生データであるため
...
2. DB `WriteTransaction` でメタデータとprimitive名前索引を反映して commit
3. commit 成功後に一時ファイルを最終配置へ rename
4. import対象page ID群のprompt候補を差分同期する
5. DB commit前の失敗時は一時ディレクトリを破棄する
...
prompt候補同期はDB commitとアセット本配置の後に別transactionで実行する。
同期失敗時は、すでにcommitされたページ正本、primitive名前索引、
アセット配置を巻き戻さず、同期未完了を呼び出し元へ返す。
復旧には共通再構成経路を使用し、import専用のprompt全件再生成処理は設けない。
...
7. 全リビジョンのfront matter検証
8. import対象内のprompt名重複検出
9. import対象と既存ページ間のprompt名重複検出
10. prompt名重複時のページ正本・名前索引の原子的な非反映
11. import対象page ID群のprompt候補差分同期
...
11. promptページを含むbackup / migrate round-trip
12. prompt派生データと名前索引がarchiveへ収録されないこと
13. import後にpromptページだけへ候補が生成されること
14. prompt候補同期失敗後に共通再構成で復旧できること
