...
- Markdown解析層は、共通front matter解析結果からresource用途を分類し、
  `resource_id`、`name`、`description`、`mime_type`を公開する
- database層は、resource URI逆引き索引、resource候補、保存時同期、
  初期構築、再構成を担当する
- MCP service層は、read scope、path prefix制約、URI検証、
...
ページ由来resourceはresource IDで識別する。resource IDからpage_idへの逆引きには、
`RESOURCE_URI_INDEX_TABLE`を使用する。
構築状態は`RESOURCE_URI_INDEX_STATE_TABLE`で管理する。

- 概念上の構造は`resource_id => page_id`とする
- create、put、amend、append、rollbackでは、保存予定のlatest sourceから
  resource IDを生成する
- `mcp.resource_id`が存在する場合はその値を使用する
- `mcp.resource_id`が未指定の場合はcurrent pathから先頭の`/`を除いた値を使用する
- 一意性確認、旧resource ID解放、新resource ID登録はページ正本と同じredb write transactionで行う
- resource指定解除、明示resource ID変更、別primitiveへの変更では旧resource IDを解放する
- soft deleteではresource ID予約を維持し、hard deleteで解放する
- importでは、import対象内および既存ページとのresource ID重複をcommit前に検出する
...
候補には`resource_id`、`name`、`description`、`mime_type`だけを保存し、
...
- 単一または複数page IDの候補除去
- 候補と最新ページ状態を合流する一覧取得
- resource IDから公開可能な最新ページソースを取得する読取
...
公開候補から除外する。current pathはMCPの`Resource`へ公開しないが、
ページ由来resourceのpath prefix認可条件としてservice層で使用する。
...
- 明示`mcp.resource_id`を持つページのrenameではURI予約を維持する
- `mcp.resource_id`未指定ページのrenameでは、移動後current path由来のresource IDへURI予約を更新する
- renameではresource候補を書き換えず、一覧取得時の最新ページ状態との合流でpathへ追従する
- soft deleteではresource候補とURI予約を維持する
- undeleteでは既存候補を使用し、path由来resource IDが必要な場合はURI予約を復元後pathへ更新する
- 単一、再帰、migrate exportのhard deleteでは、正本削除transaction内で
...
`DatabaseManager::get_resource_source_by_id(resource_id)`は、
resource IDからページ由来resourceの最新ソースを解決する。
...
latest source欠落、front matter解析失敗、resource ID重複、DB書込み失敗時は
