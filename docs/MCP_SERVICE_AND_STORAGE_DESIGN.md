...
- `docs/MCP_RESOURCE_SPECS.md`
  - resourcesのURI、field、contents、公開エラーの外部契約を確認する場合に参照する
...
## 4. MCP promptsのサービス・永続化設計

### 4.1 責務境界

MCP promptsでは、ページソース内のfront matterと本文を正本とし、
一覧用情報を再構成可能な派生データとして扱う。

- Markdown解析層は、共通front matter解析結果からprompt用途を分類し、
  `name`、`description`、`system`、`arguments`を公開する
- database層は、primitive名前索引、prompt候補、保存後同期、
  初期構築、再構成を担当する
- MCP service層は、read scope、公開状態、最新ソースの再検証、
  引数検証、placeholder展開、message文字列生成を担当する
- MCP server層は、LuWiki内部モデルをrmcp公開型へ最終変換する
- database層の永続化型とAPIへrmcpのprotocol型を持ち込まない

### 4.2 primitive共通名前索引

promptは`mcp.name`で識別する。primitive内の一意性と名前からpage_idへの
逆引きには、`MCP_PRIMITIVE_NAME_TABLE`を使用する。
構築状態は`MCP_PRIMITIVE_NAME_STATE_TABLE`で管理する。

- 概念上の構造は`(primitive, name) => page_id`とする
- create、put、amend、append、rollbackでは、保存予定のlatest sourceから
  名前キーを生成する
- 一意性確認、旧名解放、新名登録はページ正本と同じredb write transactionで行う
- prompt指定解除、名前変更、別primitiveへの変更では旧prompt名を解放する
- soft deleteでは名前予約を維持し、hard deleteで解放する
- importでは、import対象内および既存ページとの名前重複をcommit前に検出する
- 重複を検出した場合はページ保存またはimport全体をrollbackする

`prompts/get`は名前索引からpage_idを直接解決する。名前索引、ページ状態、
latest revision、latest sourceは単一redb read transaction内で取得する。
prompt候補テーブルは名前解決や本文取得には使用しないため、候補同期の欠損だけを
理由として`prompts/get`を失敗させない。

### 4.3 prompt候補派生データ

`PROMPT_CANDIDATE_TABLE`は`page_id => PromptCandidateEntry`の構造とする。
候補には`name`、`description`、`system`、定義順を維持した`arguments`だけを
保存し、本文、path、deleted、draft、latest revisionは保存しない。

database managerは次の操作を提供する。
...
候補同期は実行時点のlatest sourceを読み、prompt候補を再計算する。
最新ソースがpromptの場合はupsertし、通常ページ、template、resource、
またはMCP primitive指定なしの場合はremoveする。同期処理は冪等とし、
古い保存処理から遅れて実行された場合もlatest sourceを基準とする。

一覧取得では候補を`PAGE_INDEX_TABLE`と合流し、current pathをdatabase内部の
`PromptCandidateListEntry`へ保持する。取得不能、deleted、draftのページは
公開候補から除外する。current pathとpage_idはMCPの`Prompt`へ公開せず、
ページ用path prefixの認可条件にも使用しない。

候補同期時および一覧取得時の名前重複検出は、primitive名前索引との不整合を
黙って公開しないための防御として維持する。

### 4.4 保存・状態変更・importの処理順

create、put、amend、append、rollbackは次の順で処理する。

1. ページ正本とprimitive名前索引を同じwrite transactionで更新する
2. 正本transactionをcommitする
3. 最新ページソースからprompt候補を別transactionで同期する

候補同期に失敗した場合、保存済みのページ正本とprimitive名前索引は
巻き戻さず、呼び出し元へ同期未完了を返す。復旧にはprompt再構成を使用する。

状態変更ごとの扱いは次のとおり。

- renameでは候補と名前索引を変更しない
- soft deleteでは候補と名前予約を維持する
- undeleteでは既存候補と名前予約を使用し、公開時の状態合流で再公開する
- 単一、再帰、migrate exportのhard deleteでは、正本削除transaction内で
  primitive名を解放し、commit後の別transactionで候補を除去する

importは次の順で処理する。

1. 検証済みbundleをDBへ投入する
2. アセットを本配置へ反映する
3. import対象page ID群のprompt候補を差分同期する

import完了後に常に全件再構成する方式は採用しない。対象ID群の同期に失敗した場合は、
共通再構成を運用上の復旧手段として使用する。

### 4.5 primitive名前索引の初期構築

名前索引の構築状態が未登録の場合は、DB初期化と同じwrite transactionで
初期構築する。

- `PAGE_INDEX_TABLE`に存在する非draftページのlatest sourceを解析する
- soft deleteページも名前予約の構築対象に含める
- 全ページの解析とprimitive内の名前重複検査に成功してから索引を置換する
- 索引置換と構築済みversion 1の登録を同じtransactionで行う
- latest source欠落、front matter解析失敗、名前重複、DB操作失敗時は
  部分commitせず、DBオープンを失敗させる
- version 1が登録済みの場合は索引を変更せず、初期化を冪等に成功させる

### 4.6 prompt単独再構成

`DatabaseManager::rebuild_prompt_candidates()`は単一write transactionで次を行う。

1. 全ページのprompt候補と名前索引を収集・検証する
2. 全件の検証成功後にprompt候補テーブルを置換する
3. prompt用primitive名前索引を置換する
4. primitive名前索引の構築状態をversion 1へ更新する
5. transactionをcommitする

`PromptRebuildData`、`collect_prompt_candidates_in_txn()`、
`replace_prompt_candidates_in_txn()`により、収集・検証と置換を分離する。
保存後差分同期と全件再構成は
`build_prompt_candidate_entry_from_source()`による解析・射影を共有する。

soft delete済みpromptは候補と名前予約の再構成対象に含める。draft、通常ページ、
templateページ、resourceページはprompt候補へ投入しない。同じページ正本状態に
対する再構成は冪等とする。

latest source欠落、front matter解析失敗、prompt名重複、DB書込み失敗時は
commitせず、既存のprompt候補、名前索引、構築状態を維持する。

### 4.7 全派生データの共通再構成

`DatabaseManager::rebuild_all_derived_data()`は単一write transactionを所有し、
templates、prompts、resourcesを同時に再構成する。

1. template候補を収集・検証する
2. prompt候補とprompt用primitive名前索引を収集・検証する
3. resource候補とresource URI逆引き索引を収集・検証する
4. 全対象の検証成功後にtemplate候補を置換する
5. prompt候補、名前索引、構築状態を置換する
6. resource候補、URI逆引き索引、構築状態を置換する
7. transactionをcommitする

template単独再構成も、候補の収集・検証と置換を分離した単一transaction方式とする。
`template_root`はtemplateのlegacy fallbackにだけ使用し、promptおよびresourceには使用しない。
`all`はfront matter由来のredb内派生データだけを対象とし、Tantivyなど
redb外の派生データを含めない。

いずれかの対象で失敗した場合はcommitせず、他の対象を含む既存状態を維持する。
再構成処理はDB責務に限定し、MCP session、transport peer、rmcp型へ依存しない。
再構成、保存後同期、削除からprompt集合変更通知およびresource集合変更通知を送信しない。
クライアントは必要に応じて`prompts/list`または`resources/list`を再取得する。

### 4.8 将来拡張との境界

M5のtagsを含む将来の派生データは、各マイルストーンで保存・再構成責務を追加する。
M4ではtagsの詳細を先行確定しない。

## 5. MCP resourcesのサービス・永続化設計

### 5.1 責務境界

MCP resourcesでは、ページソース内のfront matterと本文を正本とし、
一覧用情報およびURI逆引き情報を再構成可能な派生データとして扱う。

- Markdown解析層は、共通front matter解析結果からresource用途を分類し、
  `resource_path`、`name`、`description`、`mime_type`、`resource_acl`を公開する
- database層は、resource URI逆引き索引、resource候補、保存時同期、
  初期構築、再構成を担当する
- MCP service層は、read scope、resource ACL、URI検証、
  固定組み込みresourceまたはページ由来resourceへの分岐、最新ソースの再検証、
  front matter除去後本文の取得、MIME type補完を担当する
- MCP server層は、LuWiki内部モデルをrmcp公開型へ最終変換する
- database層の永続化型とAPIへrmcpのprotocol型を持ち込まない

固定組み込みresourceはページ正本を経由しない読み取り専用resourceとして扱う。
ページ由来resourceは本章のresource候補およびURI逆引き索引を使用する。

### 5.2 resource URI逆引き索引

ページ由来resourceはresource pathで識別する。resource pathからpage_idへの逆引きには、
`RESOURCE_URI_INDEX_TABLE`を使用する。
構築状態は`RESOURCE_URI_INDEX_STATE_TABLE`で管理する。

- 概念上の構造は`resource_path => page_id`とする
- create、put、amend、append、rollbackでは、保存予定のlatest sourceから
  resource pathを生成する
- `mcp.resource_path`が存在する場合はその値を使用する
- `mcp.resource_path`が未指定または`null`の場合はcurrent pathから
  `/pages/<page-path-without-leading-slash>`を使用する
- 一意性確認、旧resource path解放、新resource path登録はページ正本と同じredb write transactionで行う
- resource指定解除、明示resource path変更、別primitiveへの変更では旧resource pathを解放する
- soft deleteではresource path予約を維持し、hard deleteで解放する
- importでは、import対象内および既存ページとのresource path重複をcommit前に検出する
- rollback、amend、undeleteでも保存または復元予定のlatest sourceに対して同じ重複規則を適用する
- 重複を検出した場合はページ保存、復元、import全体をrollbackする

`resources/read`はURI索引からpage_idを直接解決する。URI索引、ページ状態、
latest revision、latest sourceは単一redb read transaction内で取得する。
resource候補テーブルはURI解決や本文取得には使用しないため、候補同期の欠損だけを
理由として`resources/read`を失敗させない。

`is_resource_uri_index_ready()`は`RESOURCE_URI_INDEX_STATE_TABLE`のversionを確認し、
resources capabilityの公開条件として使用する。

### 5.3 resource候補派生データ

`RESOURCE_CANDIDATE_TABLE`は`page_id => ResourceCandidateEntry`の構造とする。
候補には`resource_path`、`name`、`description`、`mime_type`、`resource_acl`を保存し、
本文、path、deleted、draft、latest revisionは保存しない。
...
- 単一または複数page IDの候補除去
- 候補と最新ページ状態を合流する一覧取得
- resource pathから公開可能な最新ページソースを取得する読取
- resource URI索引のreadiness確認

候補同期は実行時点のlatest sourceを読み、resource候補を再計算する。
最新ソースがresourceの場合はupsertし、通常ページ、template、prompt、
またはMCP primitive指定なしの場合はremoveする。同期処理は冪等とし、
古い保存処理から遅れて実行された場合もlatest sourceを基準とする。

一覧取得では候補を`PAGE_INDEX_TABLE`と合流し、current pathをdatabase内部の
`ResourceCandidateListEntry`へ保持する。取得不能、deleted、draftのページは
公開候補から除外する。current pathはMCPの`Resource`へ公開せず、
ページ由来resourceのページ用path prefix認可条件にも使用しない。

候補の`mime_type`が未指定の場合、一覧取得時および本文取得時に
`text/markdown`を既定値として補完する。

### 5.4 保存・状態変更・importの処理順

create、put、amend、append、rollbackは次の順で処理する。

1. ページ正本とresource URI逆引き索引を同じwrite transactionで更新する
2. 正本transactionをcommitする
3. 最新ページソースからresource候補を別transactionで同期する

候補同期に失敗した場合、保存済みのページ正本とresource URI逆引き索引は
巻き戻さず、呼び出し元へ同期未完了を返す。復旧にはresource再構成を使用する。

状態変更ごとの扱いは次のとおり。

- 明示`mcp.resource_path`を持つページのrenameではURI予約を維持する
- `mcp.resource_path`未指定ページのrenameでは、移動後current path由来のfallback resource pathへURI予約を更新する
- renameではresource候補を書き換えず、一覧取得時の最新ページ状態との合流でpathへ追従する
- soft deleteではresource候補とURI予約を維持する
- undeleteでは既存候補を使用し、fallback resource pathが必要な場合はURI予約を復元後pathへ更新する
- 単一、再帰、migrate exportのhard deleteでは、正本削除transaction内で
  URI予約を解放し、commit後の別transactionで候補を除去する

importは次の順で処理する。

1. 検証済みbundleをDBへ投入する
2. アセットを本配置へ反映する
3. import対象page ID群のresource候補を差分同期する

import完了後に常に全件再構成する方式は採用しない。対象ID群の同期に失敗した場合は、
共通再構成を運用上の復旧手段として使用する。

### 5.5 resource取得用DB API

`DatabaseManager::get_resource_source_by_path(resource_path)`は、
resource pathからページ由来resourceの最新ソースを解決する。

単一redb read transaction内で次を行う。

1. `RESOURCE_URI_INDEX_TABLE`からpage_idを取得する
2. `PAGE_INDEX_TABLE`からページ状態、latest revision、current pathを取得する
3. draftまたはdeletedの場合は公開不能として分類する
4. `PAGE_SOURCE_TABLE`からlatest sourceを取得する
5. 解決結果を`ResourceSourceLookupResult`として返す

結果分類は次のとおりとする。

- `Found`
  - current path、latest revision、latest sourceを保持する
- `NotFound`
  - URI索引にresource pathが存在しない
- `Unavailable`
  - ページは存在するがdraftまたはdeletedで公開対象外
- `Inconsistent`
  - URI索引のowner、ページ状態、latest sourceが不整合

service層は`NotFound`および`Unavailable`をnot foundへ写像し、
`Inconsistent`およびDB失敗をinternal errorへ写像する。

### 5.6 resource単独再構成

`DatabaseManager::rebuild_resource_candidates()`は単一write transactionで次を行う。

1. 全ページのresource候補とURI逆引き索引を収集・検証する
2. 全件の検証成功後にresource候補テーブルを置換する
3. resource URI逆引き索引を置換する
4. resource URI索引の構築状態をversion 1へ更新する
5. transactionをcommitする

`ResourceRebuildData`、`collect_resource_candidates_in_txn()`、
`replace_resource_candidates_in_txn()`により、収集・検証と置換を分離する。
保存後差分同期と全件再構成は
`build_resource_candidate_entry_from_source()`による解析・射影を共有する。

soft delete済みresourceは候補とURI予約の再構成対象に含める。draft、通常ページ、
templateページ、promptページはresource候補へ投入しない。同じページ正本状態に
対する再構成は冪等とする。

latest source欠落、front matter解析失敗、resource path重複、DB書込み失敗時は
commitせず、既存のresource候補、URI逆引き索引、構築状態を維持する。

### 5.7 全派生データ再構成との関係

`DatabaseManager::rebuild_all_derived_data()`はresource候補とURI逆引き索引を
templates、promptsと同じwrite transaction内で再構成する。

`derived rebuild --target all`はtemplates、prompts、resourcesのいずれかで失敗した場合、
全対象をcommitせず、既存状態を維持する。`derived rebuild --target resources`は
resource候補、URI逆引き索引、URI索引構築状態だけを再構成する。

`template_root`はtemplateのlegacy fallbackにだけ使用し、resourceには使用しない。

### 5.8 通知非対応

M4初期版では再構成、保存後同期、rename、削除、importから
resource集合変更通知を送信しない。
クライアントは必要に応じて`resources/list`を再取得する。
