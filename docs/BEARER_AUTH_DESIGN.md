...
- スコープは拡張可能性を確保しつつ、初期実装では `read` / `write` の2種に対応する
...
Bearer認証成功時でも、トークンID、トークン平文、TTL延長有無などの Bearer 固有メタ情報は後続ハンドラへ公開しない。Bearer固有の更新やレスポンスヘッダ付与は認証処理側で閉じる。
...
認証種別そのもの、`token_id`、更新前後の有効期限、TTL延長判定結果は認証文脈へ含めない。
...
3. ここまでを満たした後で、必要な場合のみ `X-Lock-Authentication` を検証する
...
Bearerトークンのスコープ種別は、拡張可能な列挙として表現する。初期実装では `read` と `write` の2種のみを扱うが、将来の追加を妨げない構造とする。
...
#### 5.3.4 `write` が `read` を包含する判定規則

初期実装では、`write` は `read` を包含する。判定は保存時の正規化ではなく、スコープ集合型の包含判定インタフェースで扱う。
...
この判定により、保存内容は単純な集合のまま保ちつつ、外部仕様で定義された `write` の上位互換性を満たす。
...
- ページ更新、削除、ロック取得、ロック延長、ロック解除、アセット更新系 API は `write`
...
`token list` の状態表示欄については、`docs/CLI_SPECS.md` の記述に従って設計を進めるが、以下は暫定事項として扱う。
...
- 状態表示欄
  - スコープ省略表現は `scopes` から導出する
  - 失効状態表示は `revoked` から導出する
  - 期限切れ表示を含める場合は `expire_at` と一覧取得時点の現在時刻比較から導出する
- 詳細表示
  - 最終更新日時表示は `updated_at` を用いる
  - 失効状態表示は `revoked` をそのまま用いる
...
- 状態表示欄の判読性
  - 短縮表現だけで `read` / `write` の区別が即座に分かるか
  - 失効状態が短縮表現内で見落とされにくいか
  - 期限切れ状態を短縮表現へ含めた場合に可読性を損なわないか
- 表示順と情報量
  - 状態表示欄、`token_id`、対象ユーザ名、有効期限の並びが運用上見やすいか
  - スコープと失効状態のどちらを先に見せると識別しやすいか
...
- `write` 要求 API に `write` トークンでアクセスできること
- `write` 要求 API に `read` トークンでアクセスすると 403 になること
- Basic認証では `read` / `write` いずれの要求も通過すること
...
- `token create` で指定したスコープ、TTL、任意名が保持されること
...
- `token list` の状態表示仕様は `docs/CLI_SPECS.md` に従って実装設計を進める
- 状態表示欄の省略表現や詳細表示粒度は暫定事項として扱い、使用感評価後に見直し可能とする
...
- 管理項目名
  - Bearerトークン管理情報の主要項目は `token_id` / `user_id` / `scopes` / `created_at` / `updated_at` / `ttl` / `expire_at` / `revoked` / `name` で統一した
