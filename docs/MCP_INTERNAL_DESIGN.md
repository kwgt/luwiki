...
ページ由来resourceはBearerのread scopeとresource ACLを要求し、
ページ用path prefix制約は適用しない。
...
1. Markdown解析層
   ...
   - `resource_path`、`name`、`description`、`mime_type`、`resource_acl`を読み取り専用モデルとして公開する
2. database層
   - resource候補、resource URI逆引き索引、latest source取得を提供する
   - resource URI索引readinessを保持し、capability公開条件へ接続する
3. `McpService`
   - read scope、resource ACL、URI検証、固定組み込みresource分岐、cursor、本文取得を処理する
