...
3. ページ由来resourceではcandidateのcurrent pathをpath prefix制約の判定対象にすること
4. `resources/list`ではcurrent pathがpath prefix範囲外のページ由来resourceを除外すること
5. `resources/read`ではcurrent pathがpath prefix範囲外のページ由来resourceを
   `resource not found`へ秘匿すること
6. resources応答へcurrent path、page ID、prefix判定情報を公開しないこと
7. 同じBearerによる`get_page`等のpathベースtoolsには従来のprefix制約を
   維持すること
...
2. readではauthority不一致、未知builtin、存在しないページ由来resource、
   soft delete、draft、hard delete、path prefix範囲外を同じ
   `resource not found`へ秘匿すること
3. URI索引とlatest sourceのprimitiveまたはresource ID不一致を内部エラーとし、
   別候補を探索しないこと
...
- service単体・結合テスト
  - scope、ReadOnly、固定組み込みresourceのpath prefix非適用、
    ページ由来resourceのpath prefix適用、状態フィルタ
- serverテスト
  - protocol error変換、rmcp公開型へのpath・page ID非露出
- transport統合テスト
  - HTTP 401、handshake済みsession、Bearerとsessionの組合せ
