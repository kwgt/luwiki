...
- `docs/MCP_RESOURCE_SPECS.md`
  - MCP resourcesの認証、認可、path prefix、秘匿方針
...
## 10. MCP promptsの認証・認可観点

### 10.1 transport認証

1. Authorization欠落をHTTP 401、reason `missing bearer token for MCP`で拒否すること
2. 存在しないBearerおよび失効済みBearerをHTTP 401、
   reason `unauthorized`で拒否すること
3. transport認証失敗時はprompts handlerへ到達しないこと
4. transport認証失敗時はJSON-RPC result・error、prompt一覧、prompt名、
   本文、ページ情報を返さないこと

### 10.2 scopeとReadOnly

1. `McpOperation::ListPrompts`と`GetPrompt`がread操作であること
2. read scopeを持つ認証文脈で`prompts/list`と`prompts/get`を許可すること
3. legacy write scopeのread包含を維持すること
4. append scopeだけの認証文脈では両prompts操作を拒否すること
5. scope不足はHTTP 401ではなくJSON-RPC `InvalidRequest`、
   `forbidden`、`operation is not allowed`へ変換すること
6. `ReadOnly`属性ユーザもread scopeがあれば両prompts操作を利用できること

### 10.3 path prefix非適用

1. prompts操作へページ用path prefix制約を適用しないこと
2. prefix範囲外ページ由来promptも名前で一覧・取得できること
3. candidateのcurrent pathを認可条件へ使用しないこと
4. prompts応答へcurrent path、page ID、prefix判定情報を公開しないこと
5. 同じBearerによる`get_page`等のpathベースtoolsには従来のprefix制約を
   維持すること

### 10.4 ページ状態と秘匿

1. listではsoft delete、draft、ページ索引を取得できないorphan候補を、
   ソート、cursor、50件上限より先に除外すること
2. getではsoft delete、draft、hard delete、索引先欠損を同じ
   `prompt not found`へ秘匿すること
3. 名前索引とlatest sourceの用途・名前不一致を内部エラーとし、
   別候補を探索しないこと

### 10.5 実施レイヤ分割

- service単体・結合テスト
  - scope、ReadOnly、path prefix非適用、状態フィルタ
- serverテスト
  - protocol error変換、rmcp公開型へのpath・page ID非露出
- transport統合テスト
  - HTTP 401、handshake済みsession、Bearerとsessionの組合せ
- 既存tool回帰テスト
  - pathベースtoolsのprefix制約維持

## 11. MCP resourcesの認証・認可観点
...
2. 存在しないBearerおよび失効済みBearerをHTTP 401、
   reason `unauthorized`で拒否すること
3. transport認証失敗時はresources handlerへ到達しないこと
4. transport認証失敗時はJSON-RPC result・error、resource一覧、
   resource URI、本文、ページ情報を返さないこと

### 11.2 scopeとReadOnly

1. `McpOperation::ListResources`と`ReadResource`がread操作であること
2. read scopeを持つ認証文脈で`resources/list`と`resources/read`を許可すること
3. legacy write scopeのread包含を維持すること
4. append scopeだけの認証文脈では両resources操作を拒否すること
5. scope不足はHTTP 401ではなくJSON-RPC `InvalidRequest`、
   `forbidden`、`operation is not allowed`へ変換すること
6. `ReadOnly`属性ユーザもread scopeがあれば両resources操作を利用できること

### 11.3 path prefix非適用とresource ACL

1. 固定組み込みresourceへページ用path prefix制約を適用しないこと
2. path prefix範囲外でも固定組み込みresourceを一覧・取得できること
3. ページ由来resourceにもページ用path prefix制約を適用しないこと
4. path prefix範囲外でも、read scopeとresource ACLを満たすページ由来resourceを一覧・取得できること
5. `resources/list`ではresource ACL非許可のページ由来resourceを一覧から除外すること
6. `resources/read`ではresource ACL非許可のページ由来resourceを
   `resource not found`へ秘匿すること
7. resources応答へcurrent path、page ID、prefix判定情報を公開しないこと
8. 同じBearerによる`get_page`等のpathベースtoolsには従来のprefix制約を
   維持すること
...
1. listではsoft delete、draft、ページ索引を取得できないorphan候補を、
   ソート、cursor、50件上限より先に除外すること
2. readではauthority不一致、未知builtin、存在しないページ由来resource、
   soft delete、draft、hard delete、resource ACL非許可を同じ
   `resource not found`へ秘匿すること
3. URI索引とlatest sourceのprimitiveまたはresource path不一致を内部エラーとし、
   別候補を探索しないこと
4. URI形式不正と非公開resourceが別失敗分類として扱われること

### 11.5 実施レイヤ分割

- service単体・結合テスト
  - scope、ReadOnly、固定組み込みresourceのpath prefix非適用、
    ページ由来resourceのpath prefix非適用、resource ACL、状態フィルタ
- serverテスト
  - protocol error変換、rmcp公開型へのpath・page ID非露出
- transport統合テスト
  - HTTP 401、handshake済みsession、Bearerとsessionの組合せ
