...
- 初期段階は非 SSE / 非 Streaming を維持してよいが、`initialize` を含む標準 MCP handshake は必ず通す
- endpoint は `/mcp` の単一 endpoint とする

この方針の目的は、MCP 互換性をアプリ固有実装より優先し、
一般的な AI エージェントが `initialize` から接続できる状態を最小条件として固定することにある。

現行実装は `src/mcp/transport.rs` が独自の `tool` / `arguments` JSON を直接受理しており、
標準的な JSON-RPC `initialize` を受け付けない。
そのため、transport 自前実装は継続せず、公開クレートへ置き換える方針とする。
...
- `GET`
  - 採用クレートと非 SSE 方針の整合が取れる場合のみ許可する
  - 不要な場合は `405 Method Not Allowed` を返してよい
- `DELETE`
  - session 管理を導入しない段階では未サポートとしてよい
  - ただし採用クレートが session 終了手順を前提とする場合は、その要求に従う
...
- `initialize`、`tools/list`、`tools/call` の message 形式解釈
...
初期段階では非 SSE を維持してよいが、
将来の SSE 対応や session 管理追加を阻害しないよう、以下を守る。

- transport adapter と service 層を密結合にしない
- `/mcp` endpoint 自体は維持し、内部実装だけを差し替え可能にする
- SSE や session 管理は公開クレートの更新で吸収できる構造を優先する
...
- Streamable HTTP 準拠を維持しつつ、`initialize` から `tools/call` まで標準 handshake を通す
