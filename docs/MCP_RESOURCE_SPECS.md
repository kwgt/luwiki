...
- ページ由来resourceは`mcp.resource_path`またはcurrent path由来の
  fallback resource pathで識別する
- `mcp.resource_path`未指定または`null`の場合は、
  current pathから`/pages/<page-path-without-leading-slash>`を
  fallback resource pathとして使用する
...
luwiki://<authority><resource_path>
```

authorityはrun設定`mcp_authority`で指定する。
未指定時の既定値は`local.luwiki`とする。
...
- ページ由来resourceは`luwiki://<authority><resource_path>`形式のURIで公開する
- `resource_path`はfront matter仕様の`mcp.resource_path`制約を満たす
- `/builtin`および`/builtin/`配下は固定組み込みresource用に予約する
- `/pages`および`/pages/`配下はcurrent path由来fallback用に予約し、
  明示`mcp.resource_path`では使用できない
...
- `resources/list`では、current pathがpath prefix範囲外のページ由来resourceも、
  read scope、ページ状態、resource ACLを満たす場合は一覧へ含める
- `resources/read`では、current pathがpath prefix範囲外のページ由来resourceも、
  read scope、ページ状態、resource ACLを満たす場合は取得できる
- 同じBearerを使用するpath基準MCP toolsには、
  従来どおりpath prefix制約を適用する

### 5.4 resource ACL

- resource ACLはfront matterの`mcp.resource_acl`でresourceごとに任意指定する
- `resources/list`では`mcp.resource_acl.list`を適用し、
  非許可resourceを一覧から除外する
- `resources/read`では`mcp.resource_acl.read`を適用し、
  非許可resourceはnot foundとして扱う
- read ACLはlist ACLを継承しない
- ACL未指定の場合、その操作は許可する
- 同一操作内ではdenyをallowより優先する
- allowに一致する場合は許可する
- denyとallowのどちらにも一致しない場合は`default.<operation>`を使用する
- `default.<operation>`も未指定の場合は許可する
- ACL principalはULID形式ならtoken ID、それ以外ならtoken nameとして照合する
...
- 明示`mcp.resource_path`を持つページのrenameはresource URIを変更しない
- `mcp.resource_path`未指定ページのrenameはcurrent path由来fallback resource pathを更新する
- 一覧候補に対応するページ索引がない場合は公開しない
- URI索引の解決先ページが存在しない場合はnot foundとして扱う
- URI索引と最新ソースのprimitiveまたはresource pathが一致しない場合は
  内部不整合として扱う
...
- cursorに制御文字を許容しない
- cursorのauthorityはrun設定で解決したresource authorityと一致することを要求する
- `/builtin/<builtin_id>`またはページ由来resource pathだけを許容する
- ページ由来resource pathには`mcp.resource_path`と同じ形状制約を適用する
- cursorの最大長は`luwiki://<authority>`と512文字のresource pathを合わせた長さとする

### 8.3 snapshot

- cursorはDB snapshotまたは一覧世代を表さない
- 各要求時点の最新公開候補集合に対するURI境界として扱う
- 継続取得中の追加、削除、rename、resource path変更により、
...
- 最新ソースがresourceでない場合またはresource pathが一致しない場合は、
  別候補を探索せず内部不整合とする
...
- resource ACLで非許可のページ由来resourceはエラーではなく一覧から除外する
...
- authority不一致、未知の固定組み込みresource、存在しないページ由来resource、
  draft、soft delete、hard delete、resource ACL非許可は`not_found`とする
